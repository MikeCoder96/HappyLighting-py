...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
  ...
  public void collectInitialPrefetchPositions(int paramInt, RecyclerView.LayoutManager.LayoutPrefetchRegistry paramLayoutPrefetchRegistry) {
    ...
    boolean bool1 = false;
    int j = i;
    for (int i = bool1; i < this.mInitialItemPrefetchCount && j >= 0 && j < paramInt; i++) {
    ...
    public void assignFromViewAndKeepVisibleRect(View param1View) {
      ...
      if (this.mLayoutFromEnd) {
        ...
        if (j > 0) {
          ...
          k = m - k - n + Math.min(LinearLayoutManager.this.mOrientationHelper.getDecoratedStart(param1View) - n, 0);
          if (k < 0)
            this.mCoordinate += Math.min(j, -k); 
        } 
      } else {
        ...
        if (j > 0) {
          ...
          int m = LinearLayoutManager.this.mOrientationHelper.getDecoratedEnd(param1View);
          m = LinearLayoutManager.this.mOrientationHelper.getEndAfterPadding() - Math.min(0, i1 - i - m) - k + n;
          if (m < 0)
            this.mCoordinate -= Math.min(j, -m); 
        } 
      } 
    }
...
/* Location:              C:\Users\mikel\Desktop\Projects\LEDStripController\HappyLighting_base-dex2jar.jar!\android\support\v7\widget\LinearLayoutManager.class
